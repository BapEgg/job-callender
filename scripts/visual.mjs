...
if (hash !== "f1e964ee2428e8e5da3505b313661639495a746dd9015767200b4e6bb13f3619")
  throw Error("Approved source changed");
...
try {
  ...
  if (error.code !== "ENOENT" || !baselineOnly) throw error;
}
if (audit) {
  for (const entry of audit.referenceFiles) {
    ...
    if (
      ...
      throw Error(`Baseline changed: ${entry.file}`);
  }
  sealed = true;
}
...
function compare(a, b) {
  if (a.width !== b.width || a.height !== b.height)
    return { passed: false, dimensionsMatch: false };
  ...
  return {
    passed: changedRatio <= tolerance.maxChangedRatio,
...
function crop(png, box) {
  ...
  const width = Math.min(png.width - x, Math.ceil(box.width)),
    height = Math.min(png.height - y, Math.ceil(box.height));
  ...
  try {
    ...
    if (route.startsWith("write"))
      specs.push(
        {
          selector: "#save-state",
          text: kind === "reference" ? " 브라우저 저장" : " 탭 안에 유지",
        },
        {
          selector: ".editor-actionbar + p",
          text:
            kind === "reference"
    ...
    return await page.evaluate(
      (specs) =>
        specs.map((spec) => {
          ...
          if (!el) throw Error(`Missing exception ${spec.selector}`);
          const walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT);
          let node;
          const boxes = [];
          while ((node = walker.nextNode())) {
            const start = node.textContent.indexOf(spec.text);
            if (start < 0) continue;
            const range = document.createRange();
            range.setStart(node, start);
            range.setEnd(node, start + spec.text.length);
            for (const r of range.getClientRects())
              boxes.push({ x: r.x, y: r.y, width: r.width, height: r.height });
          }
          if (!boxes.length)
            throw Error(`Exception text changed: ${spec.selector}`);
          return { ...spec, boxes };
        }),
      specs,
    );
...
try {
  for (const width of [1440, 1280, 768, 390])
    for (const route of ["home", "jobs", "write/a1"]) {
      const key = `${route.replace("/", "-")}-${width}`;
      const captures = {};
      for (const kind of baselineOnly
        ? ["reference"]
        : ["reference", "actual"]) {
        const metadataFile = path.join(
          kind === "reference" ? baselineDir : dir,
          `${key}-${kind}.json`,
        );
        if (kind === "reference") {
          try {
            captures.reference = JSON.parse(
              await fs.readFile(metadataFile, "utf8"),
            );
            captures.reference.file = path.join(
              baselineDir,
              `${key}-reference.png`,
            );
            continue;
          } catch (error) {
            if (error.code !== "ENOENT" || sealed) throw error;
          }
          try {
            await fs.access(path.join(baselineDir, `${key}-reference.png`));
            throw Error(
              "Existing baseline image requires approval before regeneration",
            );
          } catch (error) {
        ...
        await page.screenshot({
          path: path.join(captureDir, `${key}-${kind}-viewport.png`),
          animations: "disabled",
        });
        captures[kind] = { file, geometry, errors };
        await fs.writeFile(
          metadataFile,
          JSON.stringify(captures[kind], null, 2),
        );
        await context.close();
      }
      ...
      if (!baselineOnly) {
        ...
        const referenceExceptions = await textExceptions(
            width,
            route,
            "reference",
          ),
          actualExceptions = await textExceptions(width, route, "actual");
        item.textExceptions = {
          reference: referenceExceptions,
          actual: actualExceptions,
        };
        const exceptionUnion = [...referenceExceptions, ...actualExceptions];
        mask(a, exceptionUnion);
        mask(b, exceptionUnion);
        item.dimensionsMatch = a.width === b.width && a.height === b.height;
        if (item.dimensionsMatch) {
          const diff = new PNG({ width: a.width, height: a.height });
          item.changedPixels = pixelmatch(
            a.data,
            b.data,
            diff.data,
            a.width,
        ...
        const va = PNG.sync.read(
            await fs.readFile(
              path.join(baselineDir, `${key}-reference-viewport.png`),
            ),
          ),
          vb = PNG.sync.read(
            await fs.readFile(path.join(dir, `${key}-actual-viewport.png`)),
          );
        mask(va, exceptionUnion);
        mask(vb, exceptionUnion);
        const viewport = compare(va, vb);
        if (viewport.diff)
          await fs.writeFile(
            path.join(dir, `${key}-viewport-diff.png`),
            PNG.sync.write(viewport.diff),
          );
        delete viewport.diff;
        item.viewport = viewport;
        item.regions = [];
        for (const [selector, boxes] of Object.entries(
          captures.reference.geometry,
        ))
          for (const [index, box] of boxes.entries()) {
            if (box.display === "none") continue;
            const other = captures.actual.geometry[selector]?.[index];
            const ca = crop(a, box),
              cb = other ? crop(b, other) : null;
            if (!ca) continue;
            const compared = cb ? compare(ca, cb) : { passed: false };
            delete compared.diff;
            item.regions.push({ selector, index, ...compared });
          }
        item.passed =
          item.dimensionsMatch &&
          item.changedRatio <= tolerance.maxChangedRatio &&
          item.geometryMatch &&
          viewport.passed &&
          item.regions.every((r) => r.passed) &&
      ...
      console.log(
        ...
        baselineOnly
          ...
          : item.passed
            ? "PASS"
            : `FAIL ratio=${item.changedRatio} geometry=${item.geometryMatch}`,
      );
    }
} finally {
  await browser.close();
}
if (!baselineOnly || !sealed)
  await fs.writeFile(
    path.join(dir, baselineOnly ? "baseline.json" : "report.json"),
    JSON.stringify(
      {
        hash,
        tolerance,
